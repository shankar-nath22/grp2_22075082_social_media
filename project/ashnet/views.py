...
def editprofilebegin(request,username):
    user= User.objects.get(username=username)
...
def deleteprofilebegin(request):
...
def search(request,pagetype):
    username = request.GET.get("query")
    if username !='' :
        user=User.objects.filter(username=username)
        if user.exists():
            user=user[0]
            if(pagetype=='all_posts'):
                all_posts = Post.objects.all().filter(creater=user).order_by('-date_created')
                paginator = Paginator(all_posts, 10)
                page_number = request.GET.get('page')
                if page_number == None:
                    page_number = 1
                ...
                if request.user.is_authenticated:
                    all_posts = Post.objects.filter(savers=request.user).filter(creater=user).order_by('-date_created')
                ...
                if request.user.is_authenticated:
                    following_user = Follower.objects.filter(followers=request.user).values('user')
                    all_posts = Post.objects.filter(creater__in=following_user).filter(creater=user).order_by('-date_created')
                    paginator = Paginator(all_posts, 10)
                    page_number = request.GET.get('page')
                    if page_number == None:
                        page_number = 1
